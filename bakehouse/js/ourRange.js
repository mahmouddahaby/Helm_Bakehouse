...
function drawChildrensDetails() {
  introStr =
    'Our fully-customised, fun and friendly range of specialty cakes for children are guaranteed to bring a smile to the face of the birthday boy or girl';
...
for (let index = 0; index < LScelebrationArr.length; index++) {
  ...
  CelibrationCard = new cackCards(
    LScelebrationArr[index].img,
    LScelebrationArr[index].title,
    LScelebrationArr[index].desc
  );
...
children.addEventListener('click', function call(e) {
  ...
  for (let index = 0; index < LSchildrenArr.length; index++) {
    ...
    ChildrenCard = new cackCards(
      LSchildrenArr[0].img,
      LSchildrenArr[0].title,
      LSchildrenArr[0].desc
    );
