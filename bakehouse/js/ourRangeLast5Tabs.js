...
class cackCards {
  ...
  drawFirstPageCard(e) {
    ...
    if (e == undefined || e.target.text == 'Dessert Cakes') {
      drawDetails(
        'Make your get-together an occasion to remember with one of our amazing dessert cake selections',
        'Dessert Cakes'
      );
      flag = 'true';
      cardsToDraw = '';
      ...
      drawDetails(
        'Our four pastry chefs have a passion for, well, pastry! Great for parties, our bite-size savouries are also a convenient snack on the run',
        'Savouries'
      );
      flag = 'false';
      arr = savouriesArr;
      ...
      drawDetails(
        'For tea, school lunches, birthdays, weddings, baby showers and even corporate events, our cupcakes are the ideal treat for every occasion',
        'Cupcakes'
      );
      flag = 'false';
      arr = cupcakesArr;
      ...
      drawDetails(
        "We offer a huge range of delicacies, all faithfully recreated daily from Keith's original recipes and baked on site using only the freshest ingredients",
        'Treats'
      );
      flag = 'false';
      arr = treatsArr;
  ...
  drawPages(counter) {
    ...
    for (let index = 0; index < pagesButton.length; index++) {
      ...
      pagesButton[index].addEventListener(
        'click',
        this.drawFirstPageCard.bind(this)
      );
    }
  }
}
for (let index = 0; index < LSDesertArr.length; index++) {
  ...
  dessertCard = new cackCards(
    LSDesertArr[index].img,
    LSDesertArr[index].title,
    LSDesertArr[index].desc
  );
...
savouries.addEventListener('click', function call(e) {
  ...
  for (let index = 0; index < LSSavouriesArr.length; index++) {
    ...
    savouriesCard = new cackCards(
      LSSavouriesArr[0].img,
      LSSavouriesArr[0].title,
      LSSavouriesArr[0].desc
    );
...
health.addEventListener('click', function call(e) {
  ...
  for (let index = 0; index < LSHealthArr.length; index++) {
    ...
    healthCard = new cackCards(
      LSHealthArr[index].img,
      LSHealthArr[index].title,
      LSHealthArr[index].desc
    );
...
cupcakes.addEventListener('click', function call(e) {
  ...
  for (let index = 0; index < LSCupcakesArr.length; index++) {
    ...
    cupcakeCard = new cackCards(
      LSCupcakesArr[index].img,
      LSCupcakesArr[index].title,
      LSCupcakesArr[index].desc
    );
...
treats.addEventListener('click', function call(e) {
  ...
  for (let index = 0; index < LSTreatsArr.length; index++) {
    ...
    treatsCard = new cackCards(
      LSTreatsArr[index].img,
      LSTreatsArr[index].title,
      LSTreatsArr[index].desc
    );
...
catering.addEventListener('click', function call(e) {
  ...
  for (let index = 0; index < LSCateringArr.length; index++) {
    ...
    cateringCard = new cackCards(
      LSCateringArr[index].img,
      LSCateringArr[index].title,
      LSCateringArr[index].desc
    );
  ...
  if (LSCateringArr.length == 0) {
    drawDetails(
      "When it comes to keeping your event fuss-free, there's no better choice than a selection of Keith Cakes' freshly-made catering meal options",
      'Catering'
    );
    cardsToDraw = '';
    cardsToDraw += `<li class="cardsLi">
